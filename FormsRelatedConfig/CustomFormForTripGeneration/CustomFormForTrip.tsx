...
const RenderField = ({
  ...
  switch (props.fieldType) {
    ...
    case FormFieldType.INPUT:
      return (
        <div className="flex rounded-[8px] !outline-none items-center border border-[#EBEEED] shadow-[0_1px_2px_0_rgba(16,24,40,0.05)] gap-2 bg-[#FFFFFF]">
          <FormControl>
            <Input
              placeholder={props.placeholder}
              {...field}
              className={
                className
                  ? className
                  : "!border-none rounded-[8px] bg-[#FFFFFF] text-[#7F7E83] pt-[12px] pb-[12px] pr-[14px] pl-[14px] !outline-none  focus:!ring-transparent"
              }
            />
          </FormControl>
        </div>
...
const CustomFormForTrip = (props: CustomProps) => {
      ...
      render={({ field }) => (
        <FormItem className="flex-1 !mt-2">
          {props.fieldType !== FormFieldType.CHECKBOX && label && (
            <FormLabel className="text-[#7F7E83] font-normal mt-3">
              {label}
            </FormLabel>
          )}
...
export default CustomFormForTrip;
